...
X, y = get_data_for("logit", "test")

model = get_model("logit", "logit_roc_auc_cv")
...
plt.savefig(os.path.join(get_images_path(), "matrix_logit.pdf"))
plt.clf()

RocCurveDisplay.from_estimator(model, X, y)
plt.legend("",frameon=False)
plt.savefig(os.path.join(get_images_path(), "roc_curve_logit.pdf"))
plt.clf()
...
plt.tight_layout()
plt.savefig(os.path.join(get_images_path(), "coefs_logit.pdf"))
