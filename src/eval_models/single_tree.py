...
model = get_model("trees", "single_tree_roc_auc_cv")
...
plt.savefig(os.path.join(get_images_path(), "matrix_single_tree.pdf"))
plt.clf()

RocCurveDisplay.from_estimator(model, X, y)
plt.legend("",frameon=False)
plt.savefig(os.path.join(get_images_path(), "roc_curve_single_tree.pdf"))
plt.clf()
...
graph.render(directory=os.path.join(get_images_path(), "tree"))
...
plt.tight_layout()
plt.savefig(os.path.join(get_images_path(), "single_tree_feature_importances.pdf"))
plt.clf()
